...
export class PostsQueryRepository {
    ...
    async findUserLikeStatusForPosts(postIds: PostId[], userId: UserId): Promise<PostIdWithPostUserLikeStatus[]> {
        ...
        const result: PostIdWithPostUserLikeStatus[] = postObjectIds.map(postId => {
            const postLikeFound: HydratedLikeType | undefined = postsLikes.find(post => post.parentId.toString() === postId.toString())
    ...
    private mapToPostUserInfoWithCorrectId(postLikeFound: HydratedLikeType): PostIdWithPostUserLikeStatus {
        return {
            postId: postLikeFound.parentId.toString(),
